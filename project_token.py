...
# Haal base_url en client-config op voor de 'Project' service
env_config    = API_ENVIRONMENTS[ENVIRONMENT]
client_config = API_CLIENTS["Project"][ENVIRONMENT]
base_url      = env_config["base_url"]

# Maak TokenManager-instance voor Project
...
def preload_token(retries: int = 3, delay: int = 2) -> None:
    ...
    """
    logging.info("[Startup] Vooraf token laden voor Project…")
    for attempt in range(1, retries + 1):
        try:
            project_token.ensure_token()
            logging.info(f"[Project] Token succesvol geladen op poging {attempt}")
            return
        except Exception as e:
            logging.warning(f"[Project] Token poging {attempt} mislukt: {e}")
            if attempt < retries:
                time.sleep(delay)
    logging.error("[Project] Kan geen token verkrijgen na meerdere pogingen.")
