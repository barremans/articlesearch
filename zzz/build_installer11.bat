...
rem -----------------------------------------------------------------
rem  build_installer11.bat  — onedir build + volledige Inno Setup .iss
rem -----------------------------------------------------------------
...
rem [2] Versie lezen (robuust; werkt voor ' en ")
...
>>"%PY_READV%" echo io.open(r'%VER_TXT%','w',encoding='utf-8').write(m.group(1) if m else "0.0.0")
...
rem [5] Vereisten & PyInstaller aanwezig
...
rem ======= TOT HIER werkt de app in dist\ArticleSearch_%VERSION% =======
...
rem [9] Installer-script genereren — neemt ALLES recursief mee (fixt python312.dll issue)
...
echo ✅ Installer aangemaakt: %DIST_FOLDER%\%SETUP_EXE%

:DONE
echo(
echo 📂 Output: %DIST_FOLDER%\%BUILD_FOLDER%
echo 💡 Test direct: "%DIST_FOLDER%\%BUILD_FOLDER%\%PROJECT_NAME%.exe"
