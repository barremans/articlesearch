...
:: ------------------------------------------------------------
::  build_installer11.bat  (gebaseerd op 9; vaste versie-parser)
:: ------------------------------------------------------------
...
:: PATH opschonen (venv vooraan; user Pythons eruit)
...
:: [2] Versie lezen (robuust; ondersteunt " en ')
set "PY_READV=%TEMP%\__read_version.py"
set "VER_TXT=%TEMP%\__version_out.txt"
del /q "%PY_READV%" "%VER_TXT%" >nul 2>&1
>  "%PY_READV%" echo import re,io,sys
>> "%PY_READV%" echo t=io.open(r'version.py','r',encoding='utf-8').read()
>> "%PY_READV%" echo m=re.search(r'__version__\s*=\s*["\']([^"\']+)["\']',t)
>> "%PY_READV%" echo sys.stdout.write(m.group(1) if m else "0.0.0")
"%PYTHON_EXE%" "%PY_READV%" > "%VER_TXT%" || (echo ❌ Kon versie niet bepalen via Python.& del /q "%PY_READV%" >nul 2^>^&1 & pause & exit /b 1)
...
>>"%ISS_FILE%" echo
>>"%ISS_FILE%" echo [Files]
>>"%ISS_FILE%" echo Source: "%ABS_BUILD%\*"; DestDir: "{app}"; Flags: recursesubdirs createallsubdirs ignoreversion
>>"%ISS_FILE%" echo
>>"%ISS_FILE%" echo [Icons]
...
goto DONE
:NO_ISCC
echo [FOUT] Inno Setup Compiler niet gevonden.
goto DONE
:ISCC_FAIL
