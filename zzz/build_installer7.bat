...
rem =========================
rem [0] Forceer venv (.venv voorkeur) & schoon PATH
rem =========================
...
rem PATH opschonen en venv vooraan zetten
set "PATH=%VENV_DIR%;%VENV_DIR%\..;%VENV_DIR%\Scripts;%PATH%"

rem --- Harden environment: blokkeer usersite en strip pcadmin-paden ---
...
rem ===== Diagnose + fail-fast (pcadmin detectie) + usersite status =====
...
rem [2] Versie lezen
rem =========================
for /f "tokens=2 delims== " %%v in ('findstr "__version__" version.py') do set "VERSION=%%~v"
set "VERSION=%VERSION:"=%"
set "VERSION=%VERSION: =%"
...
rem Zorg dat PyInstaller in de venv zit
...
rem =========================
rem [9] Inno Setup script genereren
rem =========================
