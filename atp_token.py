...
# Haal base_url en client-config op voor de 'Atp' service
env_config    = API_ENVIRONMENTS[ENVIRONMENT]
client_config = API_CLIENTS["Atp"][ENVIRONMENT]
base_url      = env_config["base_url"]

# Maak TokenManager-instance voor Atp
...
def preload_token(retries: int = 3, delay: int = 2) -> None:
    ...
    """
    logging.info("[Startup] Vooraf token laden voor Atp…")
    for attempt in range(1, retries + 1):
        try:
            project_token.ensure_token()
            logging.info(f"[Atp] Token succesvol geladen op poging {attempt}")
            return
        except Exception as e:
            logging.warning(f"[Atp] Token poging {attempt} mislukt: {e}")
            if attempt < retries:
                time.sleep(delay)
    logging.error("[Atp] Kan geen token verkrijgen na meerdere pogingen.")
