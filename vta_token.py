...
# Haal base_url en client-config op voor de 'vta' service
env_config    = API_ENVIRONMENTS[ENVIRONMENT]
client_config = API_CLIENTS["VTA"][ENVIRONMENT]
base_url      = env_config["base_url"]

# Maak TokenManager-instance voor vta
...
def preload_token(retries: int = 3, delay: int = 2) -> None:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            logging.info(f"[VTA] Token succesvol geladen op poging {attempt}")
            return
        except Exception as e:
            logging.warning(f"[VTA] Token poging {attempt} mislukt: {e}")
            if attempt < retries:
                time.sleep(delay)
    logging.error("[VTA] Kan geen token verkrijgen na meerdere pogingen.")
