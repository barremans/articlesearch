...
class DeliveryLinesDialog(QDialog):
    """
    Pop-up om DNL1 (leveringslijnen) te tonen.
    Boven: tabel met lijnen; Onder: vrijetekstveld (LineMemo/FreeTxt/Description).
    """
    COLS = ["LineNum", "ItemCode", "Dscription", "Quantity", "OpenQty", "Price", "LineTotal", "WhsCode"]
    ALIASES: Mapping[str, list[str]] = {
        ...
        "OpenQty": ["OpenQty", "OpenQuantity"],
        "Price": ["Price", "UnitPrice", "PriceExVAT", "PriceAfVAT"],
        "LineTotal": ["LineTotal", "LineTotalExclVAT", "TotalExclVAT"],
        "WhsCode": ["WhsCode", "Warehouse"],
    }

    def __init__(self, parent=None, *, delivery_header: dict | None = None, lines: Iterable[dict] = ()):
        super().__init__(parent)
        self.setWindowTitle(self._mk_title(delivery_header))
        self.resize(1000, 600)
        self._lines = list(lines or [])

        root = QVBoxLayout(self); root.setContentsMargins(10,10,10,10); root.setSpacing(8)

        if isinstance(delivery_header, dict):
            root.addWidget(self._make_header(delivery_header))
    ...
    def _mk_title(self, hdr: dict | None) -> str:
        if not isinstance(hdr, dict): return "Leveringslijnen"
        num = hdr.get("DeliveryNum") or hdr.get("DocNum") or ""
        date = hdr.get("DeliveryDate") or hdr.get("DocDate") or ""
        code = hdr.get("CardCode") or ""; name = hdr.get("CardName") or ""
        suffix = " | ".join([s for s in [f"Doc {num}" if num else "", str(date) if date else "", code, name] if s])
        return f"Leveringslijnen — {suffix}" if suffix else "Leveringslijnen"

    def _make_header(self, hdr: dict) -> QWidget:
        w = QWidget(); l = QHBoxLayout(w); l.setContentsMargins(0,0,0,0)
        left = QLabel(f"<b>DocNum:</b> {hdr.get('DeliveryNum', hdr.get('DocNum','-'))} &nbsp;&nbsp; "
                      f"<b>DocDate:</b> {hdr.get('DeliveryDate', hdr.get('DocDate','-'))}")
    ...
    def _on_selection_changed(self):
        row = self.table.currentRow(); text = ""
        if 0 <= row < len(self._lines):
            ln = self._lines[row]
            text = ln.get("LineMemo") or ln.get("FreeTxt") or ln.get("FreeText") or ln.get("Dscription") or ""
        self.free_txt.setPlainText(str(text))
