...
class OrderLinesDialog(QDialog):
    ...
    # kolommen die we standaard laten zien (met alias-lijst)
    COLS = ["LineNum", "ItemCode", "Dscription", "OpenQty", "Price", "OpenAmount_ExclVAT", "VAT_Amount", "OpenAmount_InclVAT", "Currency", "U_TRI_origitem"]
    ALIASES: Mapping[str, list[str]] = {
        ...
        # FreeTxt tonen we niet als kolom, maar afzonderlijk onderaan
    }

    def __init__(self, parent=None, *, order_header: dict | None = None, lines: Iterable[dict] = ()):
        ...
        self.free_txt.setPlaceholderText("Free text van geselecteerde lijn…")
    ...
    def _mk_title(self, order_header: dict | None) -> str:
        ...
        parts = ["Orderlijnen"]
        suffix = []
        if docnum: suffix.append(f"DocNum {docnum}")
    ...
    def _make_header(self, hdr: dict) -> QWidget | None:
        # bescheiden header met enkele kernvelden
        w = QWidget()
        l = QHBoxLayout(w); l.setContentsMargins(0, 0, 0, 0)
        left = QLabel(f"<b>DocNum:</b> {hdr.get('DocNum','-')} &nbsp;&nbsp; "
                      f"<b>DocDate:</b> {hdr.get('DocDate','-')}")
        right = QLabel(f"<b>Partner:</b> {hdr.get('CardCode','-')} — {hdr.get('CardName','-')}")
        l.addWidget(left); l.addStretch(1); l.addWidget(right)
        return w
    ...
    def _render_lines(self):
        ...
        if not col_order:
            ...
            self.table.setHorizontalHeaderLabels([str(k) for k in keys])
        ...
        for r, row in enumerate(rows):
            for c, label in enumerate(col_order):
                ...
                val = row.get(key, "")
                it = QTableWidgetItem("" if val is None else str(val))
                # rechts uitlijnen als het numeriek lijkt
