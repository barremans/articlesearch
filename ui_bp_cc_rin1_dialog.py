...
class CreditNoteLinesDialog(QDialog):
    """Pop-up om RIN1 (kredietnota-lijnen) te tonen."""
    ...
    def __init__(self, parent=None, *, credit_header: dict | None = None, lines: Iterable[dict] = ()):
        super().__init__(parent)
        self.setWindowTitle(self._mk_title(credit_header)); self.resize(1000, 600)
        self._lines = list(lines or [])
        root = QVBoxLayout(self); root.setContentsMargins(10,10,10,10); root.setSpacing(8)

        if isinstance(credit_header, dict): root.addWidget(self._make_header(credit_header))
    ...
    def _mk_title(self, hdr: dict | None) -> str:
        if not isinstance(hdr, dict): return "Kredietnota-lijnen"
        num = hdr.get("DocNum",""); date = hdr.get("DocDate","")
        code = hdr.get("CardCode",""); name = hdr.get("CardName","")
        suffix = " | ".join([s for s in [f"CN {num}" if num else "", str(date) if date else "", code, name] if s])
        return f"Kredietnota-lijnen — {suffix}" if suffix else "Kredietnota-lijnen"
