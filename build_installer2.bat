...
set "ZIP_PATH=%DIST_FOLDER%\%BUILD_FOLDER%.zip"
...
echo Alle assets en metadata toegevoegd. >> "%LOGFILE%"

echo [6] 📦 ZIP maken van buildmap...
powershell -Command "Compress-Archive -Path '%DIST_FOLDER%\%BUILD_FOLDER%\*' -DestinationPath '%ZIP_PATH%' -Force"

echo [7] 📝 Installer-script aanmaken...
> "%ISS_FILE%" echo [Setup]
...
>> "%ISS_FILE%" echo SetupIconFile="%ICON_PATH%"

>> "%ISS_FILE%" echo.
>> "%ISS_FILE%" echo [Files]
>> "%ISS_FILE%" echo Source: "%BUILD_FOLDER%\ArticleSearch.exe"; DestDir: "{app}"; Flags: ignoreversion
>> "%ISS_FILE%" echo Source: "%BUILD_FOLDER%\settings.json"; DestDir: "{app}"; Flags: onlyifdoesntexist
>> "%ISS_FILE%" echo Source: "%BUILD_FOLDER%\_internal\*"; DestDir: "{app}\_internal"; Flags: recursesubdirs createallsubdirs ignoreversion
>> "%ISS_FILE%" echo Source: "%BUILD_FOLDER%\assets\*"; DestDir: "{app}\assets"; Flags: recursesubdirs createallsubdirs ignoreversion
>> "%ISS_FILE%" echo Source: "%BUILD_FOLDER%\label\*"; DestDir: "{app}\label"; Flags: recursesubdirs createallsubdirs ignoreversion
>> "%ISS_FILE%" echo Source: "%BUILD_FOLDER%\logs\*"; DestDir: "{app}\logs"; Flags: recursesubdirs createallsubdirs ignoreversion
>> "%ISS_FILE%" echo Source: "%BUILD_FOLDER%\docs\*"; DestDir: "{app}\docs"; Flags: recursesubdirs createallsubdirs ignoreversion
>> "%ISS_FILE%" echo Source: "%ICON_PATH%"; DestDir: "{app}"; Flags: ignoreversion
...
echo [✔] Installer-script aangemaakt: %ISS_FILE%

echo [8] 🔨 Inno Setup compileren...
if exist "C:\Program Files (x86)\Inno Setup 6\ISCC.exe" (
    "C:\Program Files (x86)\Inno Setup 6\ISCC.exe" "%ISS_FILE%"
    echo ✅ Installer aangemaakt: %SETUP_EXE%
) else (
    echo ⚠️ Inno Setup Compiler (ISCC.exe) niet gevonden. Pas het pad aan indien nodig.
)
...
echo ✅ Build en installatiepakket afgerond.
