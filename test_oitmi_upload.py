...
import os
from settings import load_detail_modal, load_detail_qss_path, load_upload_qss_path


class ImageUploader(QWidget):
    # Signaal dat vuurt als de upload naar de API succesvol is
    ...
    def __init__(self,
                 ...
                 oitmi_id="",
                 oitmi_type="IMG"):
        # 1) Roep QWidget.__init__ aan met parent, en markeer dit widget als Window
        super().__init__(parent, Qt.Window)

        # 2) Zet WindowFlags: AlwaysOnTop, enkel Close-knop, geen drag‐of resize‐handles
        self.setWindowFlags(
            Qt.Window
        ...
        # eerst uit settings halen, anders default
        qss_path = load_upload_qss_path() or os.path.join(os.path.dirname(__file__), "assets", "css", "upload.qss")
        ...
        # 3) Optioneel: blokkeer resizen door minimum-afmetingen
        self.setMinimumWidth(700)
        self.setMinimumHeight(800)

        self.setWindowTitle("OITMI Upload Tester")

        # 4) Esc‐toets sluit het venster
        esc_shortcut = QShortcut(QKeySequence("Escape"), self)
        esc_shortcut.activated.connect(self.close)

        # 5) Bewaar alleen original_blob; blob gebruiken we pas in send_to_api()
        self.original_blob = original_blob.strip()
        self.oitmi_id = str(oitmi_id)
        self.oitmi_type = oitmi_type

        # 6) Bouw de rest van de UI
        layout = QVBoxLayout()
    ...
    def send_to_api(self):
        ...
        try:
            ...
            #  Vuurt alleen af nadat de upload écht gelukt is
            self.uploadSuccess.emit()

            # Sluit het window na succes
            self.close()
